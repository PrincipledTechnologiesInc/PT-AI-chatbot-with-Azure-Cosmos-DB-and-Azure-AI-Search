...
public class ChatService : IChatService
{
    ...
    public async Task AddProperty(Property property)
    {
        ...
        ArgumentNullException.ThrowIfNullOrEmpty(property.id);
        ArgumentNullException.ThrowIfNullOrEmpty(property.name);

        await _cosmosDbService.InsertPropertyAsync(property);
    }

    public async Task DeleteProperty(string id, string name)
    {
        ArgumentNullException.ThrowIfNullOrEmpty(id);
        ArgumentNullException.ThrowIfNullOrEmpty(name);

        await _cosmosDbService.DeletePropertyAsync(id, name);

        try
        {
            await _ragService.RemoveMemory(new Property { id = id});
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, $"Error attempting to remove memory for property id {id} (name id {name})");
        }
    }
