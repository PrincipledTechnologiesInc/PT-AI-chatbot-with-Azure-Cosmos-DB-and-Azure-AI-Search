...
az storage container create --account-name $storageAccount --name "property-policy" --only-show-errors
az storage azcopy blob upload -c property-policy --account-name $storageAccount -s "./MemorySources/*.txt" --recursive --only-show-errors
